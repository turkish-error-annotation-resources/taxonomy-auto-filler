    ...
    def mapInfFeat(err):
        ...
        for analysis in analysisList:
            ...
            if len((err.incorrText)[start:]) > 0:
                erroredMorphemeList.append("REDUNDANT_SUFFIX") # EXPANDED_ATTRUBUTE
            ...
            elif any((":Able" in tag) or (":Neces") or (":Cop") for tag in erroredMorphemeList) and (":Able" in analysisFormatLong) and (":Neces" in analysisFormatLong) and (":Cop" in analysisFormatLong):
                infFeats["mood"] = "GenNecPot" # General/Hypothetical Necessitative Potential (eg. söyleyebilmelidir, etc.)
            elif any((":Neces") or (":Cop") for tag in erroredMorphemeList) and (":Neces" in analysisFormatLong) and (":Cop" in analysisFormatLong):
                infFeats["mood"] = "GenNec" # General/Hypothetical Necessitative (eg. kaçmalıdır, etc.)
            elif any((":Neces") or (":Able") for tag in erroredMorphemeList) and (":Neces" in analysisFormatLong) and (":Able" in analysisFormatLong):
                infFeats["mood"] = "NecPot" # Necessitative Potential (eg. gidebilmeli, etc.)
            elif any(":Neces" for tag in erroredMorphemeList):
                infFeats["mood"] = "Nec" # Necessitative (eg. gitmeli, etc.)
            elif any((":Able" in tag) or (":Aor") or (":Cond") for tag in erroredMorphemeList) and (":Able" in analysisFormatLong) and (":Aor" in analysisFormatLong) and (":Cond" in analysisFormatLong):
                infFeats["mood"] = "CndGenPot" # Conditional Predicate with Generallized Modality and Potential (eg. gidebilirse, etc.)
            elif any((":Unable" in tag) or (":Able") or (":Aor") for tag in erroredMorphemeList) and (":Unable" in analysisFormatLong) and (":Able" in analysisFormatLong) and (":Aor" in analysisFormatLong):
                infFeats["mood"] = "GenPotPot" # Generallized Modality and Potential expressed twice (eg. gidemeyebilir)
            elif any((":Unable" in tag) or (":Able") or (":Aor") for tag in erroredMorphemeList) and (((":Unable" in analysisFormatLong) and (":Aor" in analysisFormatLong)) or ((":Able" in analysisFormatLong) and (":Aor" in analysisFormatLong))):
                infFeats["mood"] = "GenPot" # General/Hypothetical Potential (eg. yürüyemez, koyabilir, etc.)
            elif any((":Unable" in tag) or (":Able") for tag in erroredMorphemeList) and (":Unable" in analysisFormatLong) and (":Able" in analysisFormatLong):
                infFeats["mood"] = "PotPot" # Potential expressed twice (eg. gelemeyebileceği, etc.)
            elif any((":Aor" in tag) or (":Cond") for tag in erroredMorphemeList) and (":Aor" in analysisFormatLong) and (":Cond" in analysisFormatLong):
                infFeats["mood"] = "CndGen" # General Conditional (eg. satarsa, etc.)
            elif any((":Able" in tag) or (":Cond") for tag in erroredMorphemeList) and (":Able" in analysisFormatLong) and (":Cond" in analysisFormatLong):
            ...
            elif any((":Able" in tag) or (":Des") for tag in erroredMorphemeList) and (":Able" in analysisFormatLong) and (":Des" in analysisFormatLong):
            ...
            elif any((":Aor" in tag) or (":Cop") for tag in erroredMorphemeList) and ((":Aor" in analysisFormatLong) or (("Zero→Verb+Pres+A3sg" in analysisFormatLong) and (":Cop" in analysisFormatLong))):
